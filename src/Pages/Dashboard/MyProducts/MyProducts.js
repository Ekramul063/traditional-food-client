...
const MyProducts = () => {
    ...
    const handleFeature = (id) => {
        fetch(`https://tradional-foodie-server.vercel.app/feature-products/${id}`, {
            method: 'PUT',
        })
            .then(res => res.json())
            .then(data => {
                ...
                if (data.modifiedCount > 0) {
                    refetch()
                    toast.success('Addedn to Feature product');
                }
            })
    }
    const handleremoveFeature = (id) => {
        fetch(`https://tradional-foodie-server.vercel.app/feature-products-remove/${id}`, {
            method: 'PATCH',
        })
            .then(res => res.json())
            .then(data => {
                ...
                if (data.modifiedCount > 0) {
                    refetch()
                    toast.success('Remove from Feature product');
                }
            })
    }
    if (isLoading) {
                                            ...
                                            <div className='flex items-center'>
                                                {
                                                    product.feature === true ?
                                                        <button onClick={() => handleremoveFeature(product._id)} className='btn btn-xs mr-2 font-bold  bg-red-500 text-white border-none'><small>remove</small></button> :
                                                        <button onClick={() => handleFeature(product._id)} className='btn btn-xs mr-2 font-bold  bg-yellow-500 text-secondary border-none'><small>feature</small></button>
                                                }
                                                <label htmlFor="editProduct-modal" className='btn btn-xs text-white bg-green-500' onClick={() => setEditingProduct(product)}>Edit <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-4">
                ...
                </div>
                {
                    myProducts.map(product =>
                        <div key={product._id} className="lg:hidden md:hidden py-4 px-5  w-full  border-spacing-2  mb-3 border">
                            ...
                            {
                                product.feature === true ?
                                    <button onClick={() => handleremoveFeature(product._id)} className='btn btn-xs mr-2 font-bold  bg-red-500 text-white border-none mb-3 mt-2'><small>remove</small></button> :
                                    <button onClick={() => handleFeature(product._id)} className='btn btn-xs mr-2 font-bold  bg-yellow-500 text-secondary border-none mb-3 mt-2'><small>feature</small></button>
                            }
